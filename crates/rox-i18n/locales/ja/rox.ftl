...
settings-appearance-milkdrop-favorites = お気に入りのみ
    .description = ライブラリ全体ではなく、星を付けたプリセットからシャッフルする
...
milkdrop-preset-capped = { $count } 件中、最初の { $shown } 件を表示しています。残りを見るにはフィルタを絞ってください。
...
milkdrop-fade = フェード
    .description = 再生が止まったとき、パネルの背景へ即座に切り替えずにフェードする。一時停止中はどちらでも最後のフレームを保つ
