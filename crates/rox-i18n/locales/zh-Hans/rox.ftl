...
settings-appearance-milkdrop-favorites = 仅收藏
    .description = 从加星的预设里随机，而不是整个库
...
milkdrop-preset-capped = 共 { $count } 个匹配，显示前 { $shown } 个。收窄筛选条件即可看到其余的。
...
milkdrop-fade = 淡出
    .description = 播放停止时淡出，而不是直接切到面板背景。暂停的曲目无论如何都会停在最后一帧
