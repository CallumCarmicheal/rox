...
hundred do. And it opens several OS windows that share entity state, which is exactly
...
The price is real. gpui is pre-1.0 and changes its API often, so an upgrade is a
...
runtime sitting between the interface and the audio engine. egui would have been simpler
to work in but weaker for a heavily composed, themed desktop app, because immediate mode
gives the framework nowhere to hold per-panel state and styling, so both become
something the app rebuilds every frame.
