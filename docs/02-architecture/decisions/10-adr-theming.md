...
change to that palette goes through a single setter, which swaps the struct, re-feeds the
...
- **A derived mode.** While a track plays, each token's hue and chroma are pulled toward
  ...
  lightness fixed is what preserves the palette's contrast ladder, so the album can
...
since it happens once per track on a thread that was already loading the art, and what
it gives up is a fixed blur that can't respond to whatever ends up drawn over it.
...
Two scalars are less expressive than per-token alpha. That's deliberate: readability
against a backdrop is one property, and per-token alphas turn it into a combinatorial
...
character, and it keeps text legible over a near-black cover or a neon one, which a
wholesale extraction does not.
