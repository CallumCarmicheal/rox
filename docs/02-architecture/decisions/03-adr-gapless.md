...
Trade: one stream for the whole session is what makes gapless possible at all. Closing
the stream at the end of a track and opening a new one for the next puts a device
teardown and re-acquisition between them, and that takes long enough to hear, so the
only way to get a true gapless boundary is to never break the stream and change what's
feeding it instead. `playback-rs` packages exactly this pattern and is worth reading,
but adopting it hands the core playback loop, the part of rox that has to be right, to
a small dependency.
...
frames that aren't music, and playing them is what makes a "gapless" album click
between tracks, so the padding has to be found and dropped. Symphonia's support for that
