...
- [03 - Quit to tray](03-quit-to-tray.md) - can rox keep playing with no windows
  ...
  (git history). The ksni tray, reopen, idle, and quit paths all hold on Plasma
  Wayland. The one blocker, stock gpui 0.2.2 stopping the Linux and Windows
  event loops on last window close, is patched out in the vendored gpui.
  macOS needs nothing, the dock is the tray.
